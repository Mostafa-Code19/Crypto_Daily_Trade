...
try:
    ...
    def whenBought(update: Update, context: CallbackContext):
        if app.buyPrice:
            context.bot.send_message(chat_id=update.effective_chat.id, text=f'Time: {app.boughtTime} | Name: {app.cryptoToTrade} | Price: {app.buyPrice}')
        else:
            context.bot.send_message(chat_id=update.effective_chat.id, text=f'There is not order available')
    ...
    print('Telegram Connection Lost...')